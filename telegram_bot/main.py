...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id,
                                   text="Usa /help para ver la lista de comandos o /login para iniciar sesión")
...
async def login(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.id in logged_in_users:
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Use /help para conocer los comandos disponibles.")
...
async def email(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_valid_email(email):
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Introduzca una dirección de correo válida.")
        return EMAIL
...
async def password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if auth:
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Login fallido, verifique sus credenciales.")
...
async def my_datasets(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.id not in logged_in_users:
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Debe iniciar sesión para usar este comando.")
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id, text="Datasets sincronizados:",
                                   reply_markup=reply_markup_sync)
    await context.bot.send_message(chat_id=update.effective_chat.id, text="Datasets no sincronizados:",
                                   reply_markup=reply_markup_async)
...
async def upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not os.path.exists("telegram_bot/media/" +
                          str(update.effective_chat.id)) or len(os.listdir("telegram_bot/media/" +
                                                                           str(update.effective_chat.id))) < 1:
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Debes adjuntar primero uno o varios archivos")
        await context.bot.send_message(chat_id=update.effective_chat.id,
                                       text="Si no tienes archivos .uvl, puedes descargarte uno usando /test")
        return
...
async def tags(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    archives = "\n".join(f"- {archivo}" for archivo in os.listdir("telegram_bot/media/" +
    ...
    await update.message.reply_text("¿Estás seguro de querer subir el dataset con estos datos?",
...
async def confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "confirm_upload":
        ...
        for i, file_name in enumerate(file_list):
            ...
            data[f"feature_models-{i}-publication_type"] = str(
                context.user_data['publication_type']).replace("_", "").replace(" ", "").lower()
